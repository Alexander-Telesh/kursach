                ...
                with st.container():
                    ...
                    with col2:
                        ...
                        if st.button(f"Открыть", key=f"open_{book.id}"):
                            ...
                            # Используем правильный путь для переключения страницы
                            # В Streamlit путь должен быть относительно корня без расширения .py
                            # Пробуем разные варианты формата пути
                            page_switched = False
                            for page_path in ["pages/2_📚_Книги", "2_📚_Книги", "Книги"]:
                                try:
                            ...
                            if not page_switched:
                                # Если автоматический переход не работает, показываем сообщение
                                st.success(f"✅ Книга '{book.title}' выбрана!")
                                st.info("📚 Пожалуйста, перейдите на страницу 'Книги' через меню слева для просмотра.")
